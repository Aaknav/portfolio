 ...
 */
const KEYS = ["NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY"] as const;
...
describe("optionality", () => {
  it("boots with nothing configured at all", async () => {
    ...
    expect(env.NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY).toBeUndefined();
  });

  it("treats the empty strings in .env.example as unset, not as values", async () => {
    const env = await loadEnv({ NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY: "" });
...
describe("rejects misconfiguration", () => {
  it("rejects a key that is not a UUID", async () => {
    await expectRejection({ NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY: "not-a-uuid" });
  });

  it("rejects a Resend key pasted into the Web3Forms slot", async () => {
    await expectRejection({ NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY: "re_abc123" });
  });
});
...
describe("accepts a valid configuration", () => {
  it("passes a well-formed access key through", async () => {
    ...
    const env = await loadEnv({ NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY: key });

    expect(env.NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY).toBe(key);
  });
});
